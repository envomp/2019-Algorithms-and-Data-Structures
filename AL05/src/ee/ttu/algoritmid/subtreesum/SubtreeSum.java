...
public class SubtreeSum {
	...
	private void recc(Node root) {
		if (root.getLeft() == null && root.getRight() == null) {
			root.setSumOfAllLeft(0);
			root.setSumOfAllChildren(root.getValue());
			return;
		}
		long sum = 0;
		if (root.getLeft() != null) {
			recc(root.getLeft());
			root.setSumOfAllLeft(root.getLeft().getSumOfAllChildren());
			sum += root.getLeft().getSumOfAllChildren();
		}
		if (root.getRight() != null) {
			recc(root.getRight());
			sum += root.getRight().getSumOfAllChildren();
		}
		root.setSumOfAllChildren(sum + root.getValue());
	}
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println(rootNode.getSumOfAllLeft());
